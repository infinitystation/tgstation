...
/obj/item/weapon/grenade/plastic/suicide_act(mob/user)
	...
	user.visible_message("<span class='suicide'>[user] ‡ÍÚË‚ËÛÂÚ ·ÓÏ·Û Ë ‰ÂÊËÚ Â∏ Ì‡‰ Ò‚ÓÂÈ „ÓÎÓ‚ÓÈ!  ‡ÊÂÚÒ&#255; [user.p_they()] Ô˚Ú‡Î[user.p_e_5()] ÔÓÍÓÌ˜ËÚ¸ ÊËÁÌ¸ ‚ÏÂÒÚÂ ÒÓ ‚Á˚‚ÓÏ!</span>")

	var/message_say = "«¿ Ã»– ¡≈« »«ﬁÃ»ÕŒ !"
	if(user.mind)
		if(alert("¬€ “≈––Œ–»—“?",,"ƒ‡", "ÕÂÚ") == "ƒ‡")
			message_say = "¿ÀÀ¿’” ¿ ¡¿–!!"
			...
			if(role == "traitor" || role == "syndicate")
				...
				message_say = "¬»¬¿ Àﬂ –≈¬ŒÀﬁ÷»ﬂ!!"
			else if(user.mind.gang_datum)
				message_say = "[uppertext(user.mind.gang_datum.name)] –”À»“!!"
