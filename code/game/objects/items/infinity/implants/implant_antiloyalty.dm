...
/*/obj/item/implant/antiloyalty/activate(cause)
	if(!cause || !imp_in)
		return 0
...
/obj/item/implant/antiloyalty/implant(mob/target, mob/user, silent = 0)
	if(..())
		if(target.isloyal())
			if(!silent)
...
/obj/item/implant/antiloyalty/removed(mob/target, silent = 0, special = 0)
	if(..())
...
/obj/item/implanter/antiloyalty
	name = "implanter (anti-loyalty)"

/obj/item/implanter/antiloyalty/New()
	imp = new /obj/item/implant/antiloyalty(src)
	..()
	update_icon()
