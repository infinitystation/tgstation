...
/obj/mecha/examine(mob/user)
	...
	if(check_rights_for(user.client, R_ADMIN))
		if(occupant)
			to_chat(user, "[occupant] сидит в мехе. (<a href='?_src_=holder;adminplayerobservefollow=\ref[occupant]'>FLW</A>) (<A HREF='?_src_=holder;adminplayeropts=\ref[occupant]'>PP</A>) (<a href='?_src_=holder;freezemecha=\ref[src]'>Freeze</A>)")
		else
			to_chat(user, "Внутри меха никого нет.")
