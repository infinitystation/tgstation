...
/proc/make_mining_asteroid_secrets()
	for(var/i in 1 to GLOB.max_secret_rooms)
...
/proc/make_mining_asteroid_secret()
	...
	switch(pick(GLOB.possiblethemes))//what kind of room is this gonna be?
		if("organharvest")
		...
		if("cavein")
			...
			fluffitems = list(/obj/effect/decal/cleanable/blood=3,/obj/effect/decal/remains/human=1,/obj/item/clothing/under/overalls=1,
							  /obj/item/reagent_containers/food/snacks/grown/chili=1,/obj/item/tank/internals/oxygen/red=2)

		if("xenoden")
			theme = "xenoden"
			walltypes = list(/turf/closed/mineral/random/high_chance=1)
			floortypes = list(/turf/open/floor/plating/asteroid/basalt, /turf/open/floor/plating/beach/sand)
			treasureitems = list(/obj/item/clothing/mask/facehugger/toy)
		...
		if("speakeasy")
			...
			treasureitems = list(/obj/item/melee/transforming/energy/sword/pirate=1,/obj/item/gun/ballistic/revolver/doublebarrel=1,/obj/item/storage/backpack/satchel/flat=1,
		...
		if("plantlab")
			theme = "plantlab"
			treasureitems = list(/obj/item/gun/energy/floragun=1,/obj/item/seeds/sunflower/novaflower=2,/obj/item/seeds/tomato/blue/bluespace=2,/obj/item/seeds/tomato/blue=2,
			/obj/item/seeds/coffee/robusta=2, /obj/item/seeds/firelemon=2)
			fluffitems = list(/obj/item/twohanded/required/kirbyplants=1,/obj/structure/table/reinforced=2,/obj/machinery/hydroponics/constructable=1,
	...
	GLOB.possiblethemes -= theme //once a theme is selected, it's out of the running!
