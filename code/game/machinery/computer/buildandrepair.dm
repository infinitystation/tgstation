...
/obj/stucture/computerframe/holoframe
	density = 1
	anchored = 0
	name = "holocomputer-frame"
	icon = 'icons/obj/holo_parts.dmi'
	icon_state = "holo_0"
	var/state = 0
	var/obj/item/weapon/circuitboard/circuit = null
...
/obj/item/weapon/circuitboard/holo/power
	name = "Power Monitoring Holoconsole"
	build_path = /obj/machinery/computer/holo/monitor
	origin_tech = "programming=3"

/obj/structure/computerframe/holoframe/attackby(obj/item/P, mob/user, params)
	add_fingerprint(user)
	switch(state)
		if(0)
			if(istype(P, /obj/item/weapon/wrench))
