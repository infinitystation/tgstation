...
/obj/machinery/computer/cloning/process()
	...
	for(var/datum/data/record/R in records)
		...
		if(pod.growclone(R.fields["ckey"], R.fields["name"], R.fields["UI"], R.fields["SE"], R.fields["mind"], R.fields["mrace"], R.fields["features"], R.fields["factions"], R.fields["age"], R.fields["visual_age"]))
			records -= R
	...
	if(href_list["task"])
		...
		if(C)
			...
			if(!LAZYLEN(pods))
				...
				playsound(src, 'sound/machines/terminal_prompt_deny.ogg', 50, 0)
			else if(pod.growclone(C.fields["ckey"], C.fields["name"], C.fields["UI"], C.fields["SE"], C.fields["mind"], C.fields["mrace"], C.fields["features"], C.fields["factions"], C.fields["age"], C.fields["visual_age"]))
