...
/datum/game_mode/gang/announce()
	world << "<B>“екущий игровой режим - ¬ойна √ангстеров!</B>"
	world << "<B>∆естокая война офисных работников возникла на станции!<BR>√ангстеры -  «ахватите станцию посредством уничтожения боссов другой банды гангстеров! Ќанимайте гангстеров, убеждая их вступить в вашу банду и, при их согласии, слепите их флэш-устройствами, чтобы обратить в свою банду! <BR>—лужба Ѕезопасности - ќбеспечьте защиту всего экипажа! »дентифицируйте и остановите Ѕоссов гангстерских группировок! <BR> ќбязательно прочтите WIKI статью по данному режиму игры: http://ru.infinity.wikia.com/wiki/GangWar </B>"
...
/datum/game_mode/proc/forge_gang_objectives(var/datum/mind/boss_mind)
	...
	rival_obj.explanation_text = "”ничтожьте или прогоните со станции [(boss_mind in A_bosses) ? gang_name("B") : gang_name("A")]."
...
/datum/game_mode/proc/greet_gang(var/datum/mind/boss_mind, var/you_are=1)
	var/obj_count = 1
	if (you_are)
		boss_mind.current << "<FONT size=3 color=red><B>¬ы [(boss_mind in A_bosses) ? gang_name("A") : gang_name("B")] Ѕосс √ангстерской группировки! <BR> ќбязательно прочтите WIKI статью по данному режиму игры: http://ru.infinity.wikia.com/wiki/GangWar </B></FONT>"
...
/datum/game_mode/proc/equip_gang(mob/living/carbon/human/mob)
	...
	if (!where)
		...
		mob << "<span class='userdanger'>ѕомните, что вы можете идентифицировать только ваших соратников и вашего Ѕосса, но вы не можете видеть, кто является членом противоположной гангстерской банды. ¬ы должны скоординировать ваши действия внутри вашей банды так, чтобы они были максимально эффективны, для победы над конкурирующей группировкой.</span>"
		. += 1
...
/datum/game_mode/proc/add_gangster(datum/mind/gangster_mind, var/gang, var/check = 1)
	...
	gangster_mind.current << "<FONT size=3 color=red><B>“еперь вы член гангстерской банды [gang=="A" ? gang_name("A") : gang_name("B")] √ангстер!</B></FONT>"
	gangster_mind.current << "<font color='red'>ѕомогите вашему Ѕоссу взять контроль над станцией, одержав победу над противоположной бандой. ¬ы можете узнать вашего Ѕосса по коричневой иконке с буквой \"B\", но <B>только они знают, кто другие члены вашей банды!</B> Work with your boss to avoid attacking your own gang.</font>"
