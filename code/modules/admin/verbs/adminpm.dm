...
/client/proc/cmd_admin_pm(whom, msg)
	...
	if(irc)
		for(var/client/X in GLOB.admins)
			to_chat(X, "<font color='blue'><B>PM: [key_name(src, X, 1)]-&gt;IRC:</B> [keywordparsedmsg]</font>")
	else
		...
		for(var/client/X in GLOB.admins)
			if(X.key!=key && X.key!=recipient.key)	//check client/X is an admin and isn't the sender or recipient
				to_chat(X, "<font color='blue'><B>PM: [key_name(src, X, 0)](<a href='?_src_=holder;adminplayerobservefollow=\ref[src.mob]'>FLW</A>)-&gt;[key_name(recipient, X, 0)](<a href='?_src_=holder;adminplayerobservefollow=\ref[recipient.mob]'>FLW</A>):</B> [keywordparsedmsg]</font>" )
