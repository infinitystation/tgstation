	...
	if(adminckey || playerckey || ip || cid)
		...
		while(query_search_bans.NextRow())
			...
			switch(bantype)
				...
				if("TEMPBAN")
					typedesc = "<b>HARD TEMPBAN</b><br><font size='2'>([duration] minutes [(unbanned) ? "" : "(<a href=\"byond://?src=[REF(src)];[HrefToken()];dbbanedit=duration;dbbanid=[banid]\">Edit</a>))"]<br>Expires [expiration]</font>"
				if("JOB_PERMABAN")
				...
				if("SOFT_TEMPBAN")
					typedesc = "<b>SOFT TEMPBAN</b><br><font size='2'>([duration] minutes [(unbanned) ? "" : "(<a href=\"byond://?src=[REF(src)];[HrefToken()];dbbanedit=duration;dbbanid=[banid]\">Edit</a>))"]<br>Expires [expiration]</font>"
