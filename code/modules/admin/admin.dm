...
/datum/admins/proc/show_player_panel(mob/M in GLOB.mob_list)
	...
	if(isnewplayer(M))
		...
		body += " \[<A href='?_src_=holder;[HrefToken()];allow_respawn=[REF(M)]'>Allow Respawn</A>\] "
	...
	body += "<a href='?_src_=holder;[HrefToken()];adminplayerobservefollow=[REF(M)]'>FLW</a>\] </b><br>"
	...
	body += "<A href='?_src_=holder;[HrefToken()];softban=[REF(M)]'>Soft Ban</A> | "
	body += "<A href='?_src_=holder;[HrefToken()];newban=[REF(M)]'>HARD Ban</A> | "
	...
	body += "<A href='?_src_=holder;[HrefToken()];warnplayer=[M.ckey]'>Warn</A> | "
...
/datum/admins/proc/set_event_status()
	...
	if(!GLOB.event_on_air)
		GLOB.event_url = input(usr, "Напиши ССЫЛКУ на Ивент", "Нужна ссылка") as null|text
		if(GLOB.event_url)
			GLOB.event_on_air = !GLOB.event_on_air
		else
			to_chat(usr, "<span class='info'>Ссылка об&#255;зательна дл&#255; изменени&#255; статуса сервера (ивент) на \"Ивент\"</span>")
			return
	else
		GLOB.event_url = ""
		GLOB.event_on_air = !GLOB.event_on_air
...
/datum/admins/proc/view_event_status()
	...
	if(GLOB.event_on_air)
		to_chat(usr, "<span class='info'>На сервере [SSticker ? "идет" : "готовитс&#255;"] ивент. Подробности читайте здесь: [GLOB.event_url] или в Admin -> Admin-Notice. Также спросите коллег через asay</span>")
	else
		to_chat(usr, "Ивента нет, но, возможно, он будет. Спросите коллег")
