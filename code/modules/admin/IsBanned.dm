...
/world/IsBanned(key,address,computer_id,type,real_bans_only=FALSE)
	...
	if(CONFIG_GET(flag/ban_legacy_system))
		...
		var/datum/DBQuery/query_ban_check = SSdbcore.NewQuery("SELECT ckey, a_ckey, reason, expiration_time, duration, bantime, bantype, id, round_id FROM [format_table_name("ban")] WHERE (ckey = '[ckeytext]' [ipquery] [cidquery]) AND (bantype = 'PERMABAN' OR bantype = 'SOFT_PERMABAN' OR bantype = 'ADMIN_PERMABAN' OR ((bantype = 'TEMPBAN' OR bantype = 'ADMIN_TEMPBAN' OR bantype = 'SOFT_TEMPBAN') AND expiration_time > Now())) AND isnull(unbanned)")
