...
/client/New(TopicData)
	...
	if(holder)
		...
		spawn for()
			...
			var/datum/DBQuery/query_o_s_ins = SSdbcore.NewQuery("INSERT INTO online_score(ckey,year,month,day,sum) VALUES ('[sql_ckey]', YEAR(NOW()), MONTH(NOW()), DAYOFMONTH(NOW()), 0);")
	...
	if(GLOB.event_on_air)
		to_chat(src, "На сервере [SSticker ? "идет" : "готовится"] ивент. Подробности читайте здесь: [GLOB.event_url] или в Admin -> Admin-Notice. Также можете спросить администрацию через Adminhelp (F1)")
