...
/datum/preferences
	...
	var/age = 30						//age of character?	var/visual_age = 30
	var/visual_age = 30					//
	...
	var/list/features = list("mcolor" = "FFF", "tail_lizard" = "Smooth", "tail_human" = "None", "snout" = "Round", "horns" = "None", "ears" = "None", "wings" = "None", "frills" = "None", "spines" = "None", "body_markings" = "None", "tajaran_hair" = "Straight", "tail_tajaran" = "Default", "ears_tajaran" = "Default", "legs" = "Normal Legs")
	...
	//Ќедостатки. ћогут быть дополнены или переделаны. —пасибо ~Derven'у
	var/be_blinded = 0
	var/be_nearsight = 0
	var/be_deaf = 0
			...
			switch(href_list["preference"])
				...
				if("thair_style")
					...
					if(istype(pref_species, /datum/species/tajaran))
						new_thair_style = input(user, "Choose your character's hair style:", "Character Preference")  as null|anything in GLOB.hair_styles_tajaran
					if(new_thair_style)
						features["tajaran_hair"] = new_thair_style

				if("next_thair_style")
					if (istype(pref_species, /datum/species/tajaran))
						features["tajaran_hair"] = next_list_item(features["tajaran_hair"], GLOB.hair_styles_tajaran)

				if("previous_thair_style")
					if (istype(pref_species, /datum/species/tajaran))
						features["tajaran_hair"] = previous_list_item(features["tajaran_hair"], GLOB.hair_styles_tajaran)
				...
				if("ears_tajaran")
					var/new_ears
					new_ears = input(user, "Choose your character's ears:", "Character Preference") as null|anything in GLOB.ears_list_tajaran
					if(new_ears)
