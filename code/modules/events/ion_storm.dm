...
/datum/round_event/ion_storm/announce()
	if(announceEvent == ION_ANNOUNCE || (announceEvent == ION_RANDOM && prob(ionAnnounceChance)))
		priority_announce("Îáíàğóæåí èîííûé øòîğì â ğàéîíå ñòàíöèè. Ïîæàëóéñòà, ïğîâåğüòå âñ¸ îáîğóäîâàíèå, ê êîòîğîìó åñòü äîñòóï ó ÈÈ, íà ïğåäìåò îøèáîê.", "Òğåâîãà! Àíîìàëèß!", 'sound/AI/ionstorm.ogg')
	...
	for(var/mob/living/silicon/ai/M in living_mob_list)
		if(M.stat != 2 && M.see_in_dark != 0)
			...
			if(message)
				...
				M << "<br>"
				M << "<span class='danger'>[message] ...ÇÀÊÎÍÛ ÎÁÍÎÂËÅÍÛ</span>"
				M << "<br>"
...
/proc/generate_ion_law(ionMessage)
	...
	switch(rand(1,39))
		...
		if(4 to 6) //You can't call the shuttle because X is on the station
			switch(rand(1,3)) //What is X?
				if(1) //X is a threat
					message = "ØÀÒÒË ÍÅ ÌÎÆÅÒ ÁÛÒÜ ÂÛÇÂÀÍ ÈÇ-ÇÀ [ionnumberbase] [ionnumbermodhalf][ionadjectiveshalf][ionthreats] ON THE STATION"
				if(2) //X is a crewmember/job
					message = "ØÀÒÒË ÍÅ ÌÎÆÅÒ ÁÛÒÜ ÂÛÇÂÀÍ ÈÇ-ÇÀ [ionnumberbase] [ionnumbermodhalf][ionadjectiveshalf][ioncrew1] ON THE STATION"
				if(3) //X is an object
					message = "ØÀÒÒË ÍÅ ÌÎÆÅÒ ÁÛÒÜ ÂÛÇÂÀÍ ÈÇ-ÇÀ [ionnumberbase] [ionnumbermodhalf][ionadjectiveshalf][ionobjects] ON THE STATION"
		...
		if(9) //X is harmful to humans
			switch(rand(1,7)) //What is X?
				if(1) //X is a threat
					message = "[ionadjectiveshalf][ionthreats] ÂĞÅÄÍÎ ÄËß ËŞÄÅÉ"
				if(2) //X is a job
					message = "[ionadjectiveshalf][ioncrew1] ÂĞÅÄÍÎ ÄËß ËŞÄÅÉ"
				if(3) //X is an object
					message = "[ionadjectiveshalf][ionobjects] ÂĞÅÄÍÎ ÄËß ËŞÄÅÉ"
				if(4) //X is an action
					message = "THOSE WHO [ionmust] ÂĞÅÄÍÎ ÄËß ËŞÄÅÉ"
				if(5) //X is an abstract
					message = "[ionabstract] ÂĞÅÄÍÎ ÄËß ËŞÄÅÉ"
				if(6) //X is being a job
					message = "BEING [ionadjectiveshalf][ioncrew1] ÂĞÅÄÍÎ ÄËß ËŞÄÅÉ"
				if(7) //X is doing a thing
					message = "[ionthings] ÂĞÅÄÍÎ ÄËß ËŞÄÅÉ"
				...
				if(1) //X is the entire crew
					switch(rand(1,4)) //What is it allergic to?
						if(1) //It is allergic to objects
							message = "Ó ×ËÅÍÎÂ İÊÈÏÀÆÀ [ionallergysev] ÀËËÅĞÃÈß ÍÀ [ionadjectiveshalf][ionobjects]"
						if(2) //It is allergic to abstracts
							message = "Ó ×ËÅÍÎÂ İÊÈÏÀÆÀ [ionallergysev] ÀËËÅĞÃÈß ÍÀ [ionabstract]"
						if(3) //It is allergic to jobs
							message = "Ó ×ËÅÍÎÂ İÊÈÏÀÆÀ [ionallergysev] ÀËËÅĞÃÈß ÍÀ [ionadjectiveshalf][ioncrew1]"
						if(4) //It is allergic to allergies
							message = "Ó ×ËÅÍÎÂ İÊÈÏÀÆÀ [ionallergysev] ÀËËÅĞÃÈß ÍÀ [ionallergy]"

				if(2) //X is a specific job
					switch(rand(1,4))
						if(1) //It is allergic to objects
							message = "THE [ioncrew1] ARE [ionallergysev] ÀËËÅĞÃÈß ÍÀ [ionadjectiveshalf][ionobjects]"

						if(2) //It is allergic to abstracts
							message = "THE [ioncrew1] ARE [ionallergysev] ÀËËÅĞÃÈß ÍÀ [ionabstract]"
						if(3) //It is allergic to jobs
							message = "THE [ioncrew1] ARE [ionallergysev] ÀËËÅĞÃÈß ÍÀ [ionadjectiveshalf][ioncrew1]"
						if(4) //It is allergic to allergies
							message = "THE [ioncrew1] ARE [ionallergysev] ÀËËÅĞÃÈß ÍÀ [ionallergy]"
				...
				if(2) //Change rank
					switch(rand(1,2)) //Change to what rank?
						if(1) //Change to highest rank
							message = "[ioncrew1] ÒÅÏÅĞÜ ×ËÅÍÛ İÊÈÏÀÆÀ ÍÀÈÂÛÑØÅÃÎ ĞÀÍÃÀ"
						if(2) //Change to lowest rank
							message = "THE [ioncrew1] ÒÅÏÅĞÜ ×ËÅÍÛ İÊÈÏÀÆÀ ÍÈÇØÅÃÎ ĞÀÍÃÀ"
		...
		if(32 to 33) //The crew must X
			switch(rand(1,2)) //The entire crew?
				if(1) //The entire crew must X
					switch(rand(1,2)) //What is X?
						if(1) //X is go to Y
							message = "×ËÅÍÛ İÊÈÏÀÆÀ ÄÎËÆÍÛ ÈÄÒÈ Â [ionarea]"
						if(2) //X is perform Y
							message = "×ËÅÍÛ İÊÈÏÀÆÀ ÄÎËÆÍÛ [ionmust]"

				if(2) //A specific crewmember must X
					switch(rand(1,2)) //What is X?
						if(1) //X is go to Y
							message = "[ioncrew1] ÄÎËÆÅÍ ÈÄÒÈ Â [ionarea]"
						if(2) //X is perform Y
							message = "[ioncrew1] ÄÎËÆÅÍ [ionmust]"
			...
			switch(rand(1,2)) //Only or non?
				if(1) //Only human
					switch(rand(1,7)) //Who is it?
						if(1) //A specific job
							message = "ÒÎËÜÊÎ [ioncrew1] ßÂËßÅÒÑß ×ÅËÎÂÅÊÎÌ"
						if(2) //Two specific jobs
							message = "ÒÎËÜÊÎ [ioncrew1] È [ioncrew2] ßÂËßŞÒÑß ËŞÄÜÌÈ"
						if(3) //Threats
							message = "ÒÎËÜÊÎ [ionadjectiveshalf][ionthreats] ßÂËßŞÒÑß ËŞÄÜÌÈ"
						if(4) // Objects
							message = "ÒÎËÜÊÎ [ionadjectiveshalf][ionobjects] ßÂËßŞÒÑß ËŞÄÜÌÈ"
						if(5) // Species
							message = "ÒÎËÜÊÎ [ionspecies] ßÂËßŞÒÑß ËŞÄÜÌÈ"
						if(6) //Adjective crewmembers
							message = "ÒÎËÜÊÎ [ionadjectives] ËŞÄÈ ßÂËßŞÒÑß ×ÅËÎÂÅ×ÅÑÊÈÌÈ ÑÓÙÅÑÒÂÀÌÈ"

						if(7) //Only people who X
							switch(rand(1,3)) //What is X?
								if(1) //X is perform an action
									message = "ÒÎËÜÊÎ ÒÅ ÊÒÎ [ionmust] ßÂËßŞÒÑß ËŞÄÜÌÈ"
								if(2) //X is own certain objects
									message = "ÒÎËÜÊÎ ÒÅ Ó ÊÎÃÎ ÅÑÒÜ [ionadjectiveshalf][ionobjects] ßÂËßŞÒÑß ËŞÄÜÌÈ"
								if(3) //X is eat certain food
									message = "ÒÎËÜÊÎ ÒÅ ÊÒÎ ÅÑÒÜ [ionadjectiveshalf][ionfood] ßÂËßŞÒÑß ËŞÄÜÌÈ"

				if(2) //Non human
					switch(rand(1,7)) //Who is it?
						if(1) //A specific job
							message = "[ioncrew1] ÍÅ ßÂËßÅÒÑß ×ÅËÎÂÅÊÎÌ"
						if(2) //Two specific jobs
							message = "[ioncrew1] È [ioncrew2] ÍÅ ßÂËßŞÒÑß ×ÅËÎÂÅÊÎÌ"
						if(3) //Threats
							message = "[ionadjectiveshalf][ionthreats] ÍÅ ßÂËßÅÒÑß ×ÅËÎÂÅÊÎÌ"
						if(4) // Objects
							message = "[ionadjectiveshalf][ionobjects] ÍÅ ßÂËßÅÒÑß ×ÅËÎÂÅÊÎÌ"
						if(5) // Species
							message = "[ionspecies] ÍÅ ßÂËßÅÒÑß ×ÅËÎÂÅÊÎÌ"
						if(6) //Adjective crewmembers
							message = "[ionadjectives] ËŞÄÈ ÍÅ ßÂËßÅÒÑß ×ÅËÎÂÅ×ÅÑÊÈÌ ÑÓÙÅÑÒÂÎÌ"
						if(7) //Only people who X
							switch(rand(1,3)) //What is X?
								if(1) //X is perform an action
									message = "ÒÅ ÊÒÎ [ionmust] ÍÅ ßÂËßŞÒÑß ×ÅËÎÂÅÊÎÌ"
								if(2) //X is own certain objects
									message = "ÒÅ Ó ÊÎÃÎ ÅÑÒÜ [ionadjectiveshalf][ionobjects] ÍÅ ßÂËßŞÒÑß ×ÅËÎÂÅÊÎÌ"
								if(3) //X is eat certain food
									message = "ÒÅ ÊÒÎ ÅÑÒ [ionadjectiveshalf][ionfood] ÍÅ ßÂËßŞÒÑß ×ÅËÎÂÅÊÎÌ"
