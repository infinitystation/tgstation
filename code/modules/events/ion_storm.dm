...
/datum/round_event/ion_storm/announce()
	if(announceEvent == ION_ANNOUNCE || (announceEvent == ION_RANDOM && prob(ionAnnounceChance)))
		priority_announce("Œ·Ì‡ÛÊÂÌ ËÓÌÌ˚È ¯ÚÓÏ ‚ ‡ÈÓÌÂ ÒÚ‡ÌˆËË. œÓÊ‡ÎÛÈÒÚ‡, ÔÓ‚Â¸ÚÂ ‚Ò∏ Ó·ÓÛ‰Ó‚‡ÌËÂ, Í ÍÓÚÓÓÏÛ ÂÒÚ¸ ‰ÓÒÚÛÔ Û »», Ì‡ ÔÂ‰ÏÂÚ Ó¯Ë·ÓÍ.", "“Â‚Ó„‡! ¿ÌÓÏ‡ÎËˇ!", 'sound/AI/ionstorm.ogg')
	...
	for(var/mob/living/silicon/ai/M in living_mob_list)
		if(M.stat != 2 && M.see_in_dark != 0)
			...
			if(message)
				...
				M << "<br>"
				M << "<span class='danger'>[message] ...«¿ ŒÕ€ Œ¡ÕŒ¬À≈Õ€</span>"
				M << "<br>"
...
/proc/generate_ion_law(ionMessage)
	...
	switch(rand(1,41))
		...
		if(4 to 6) //You can't call the shuttle because X is on the station
			switch(rand(1,3)) //What is X?
				if(1) //X is a threat
					message = "ÿ¿““À Õ≈ ÃŒ∆≈“ ¡€“‹ ¬€«¬¿Õ »«-«¿ [ionnumberbase] [ionnumbermodhalf][ionadjectiveshalf][ionthreats] ON THE STATION"
				if(2) //X is a crewmember/job
					message = "ÿ¿““À Õ≈ ÃŒ∆≈“ ¡€“‹ ¬€«¬¿Õ »«-«¿ [ionnumberbase] [ionnumbermodhalf][ionadjectiveshalf][ioncrew1] ON THE STATION"
				if(3) //X is an object
					message = "ÿ¿““À Õ≈ ÃŒ∆≈“ ¡€“‹ ¬€«¬¿Õ »«-«¿ [ionnumberbase] [ionnumbermodhalf][ionadjectiveshalf][ionobjects] ON THE STATION"
		...
		if(9) //X is harmful to humans
			switch(rand(1,7)) //What is X?
				if(1) //X is a threat
					message = "[ionadjectiveshalf][ionthreats] ¬–≈ƒÕŒ ƒÀﬂ Àﬁƒ≈…"
				if(2) //X is a job
					message = "[ionadjectiveshalf][ioncrew1] ¬–≈ƒÕŒ ƒÀﬂ Àﬁƒ≈…"
				if(3) //X is an object
					message = "[ionadjectiveshalf][ionobjects] ¬–≈ƒÕŒ ƒÀﬂ Àﬁƒ≈…"
				if(4) //X is an action
					message = "THOSE WHO [ionmust] ¬–≈ƒÕŒ ƒÀﬂ Àﬁƒ≈…"
				if(5) //X is an abstract
					message = "[ionabstract] ¬–≈ƒÕŒ ƒÀﬂ Àﬁƒ≈…"
				if(6) //X is being a job
					message = "BEING [ionadjectiveshalf][ioncrew1] ¬–≈ƒÕŒ ƒÀﬂ Àﬁƒ≈…"
				if(7) //X is doing a thing
					message = "[ionthings] ¬–≈ƒÕŒ ƒÀﬂ Àﬁƒ≈…"
				...
				if(1) //X is the entire crew
					switch(rand(1,4)) //What is it allergic to?
						if(1) //It is allergic to objects
							message = "” ◊À≈ÕŒ¬ › »œ¿∆¿ [ionallergysev] ¿ÀÀ≈–√»ﬂ Õ¿ [ionadjectiveshalf][ionobjects]"
						if(2) //It is allergic to abstracts
							message = "” ◊À≈ÕŒ¬ › »œ¿∆¿ [ionallergysev] ¿ÀÀ≈–√»ﬂ Õ¿ [ionabstract]"
						if(3) //It is allergic to jobs
							message = "” ◊À≈ÕŒ¬ › »œ¿∆¿ [ionallergysev] ¿ÀÀ≈–√»ﬂ Õ¿ [ionadjectiveshalf][ioncrew1]"
						if(4) //It is allergic to allergies
							message = "” ◊À≈ÕŒ¬ › »œ¿∆¿ [ionallergysev] ¿ÀÀ≈–√»ﬂ Õ¿ [ionallergy]"

				if(2) //X is a specific job
					switch(rand(1,4))
						if(1) //It is allergic to objects
							message = "THE [ioncrew1] ARE [ionallergysev] ¿ÀÀ≈–√»ﬂ Õ¿ [ionadjectiveshalf][ionobjects]"

						if(2) //It is allergic to abstracts
							message = "THE [ioncrew1] ARE [ionallergysev] ¿ÀÀ≈–√»ﬂ Õ¿ [ionabstract]"
						if(3) //It is allergic to jobs
							message = "THE [ioncrew1] ARE [ionallergysev] ¿ÀÀ≈–√»ﬂ Õ¿ [ionadjectiveshalf][ioncrew1]"
						if(4) //It is allergic to allergies
							message = "THE [ioncrew1] ARE [ionallergysev] ¿ÀÀ≈–√»ﬂ Õ¿ [ionallergy]"
				...
				if(2) //Change rank
					switch(rand(1,2)) //Change to what rank?
						if(1) //Change to highest rank
							message = "[ioncrew1] “≈œ≈–‹ ◊À≈Õ€ › »œ¿∆¿ Õ¿»¬€—ÿ≈√Œ –¿Õ√¿"
						if(2) //Change to lowest rank
							message = "THE [ioncrew1] “≈œ≈–‹ ◊À≈Õ€ › »œ¿∆¿ Õ»«ÿ≈√Œ –¿Õ√¿"
		...
		if(32 to 33) //The crew must X
			switch(rand(1,2)) //The entire crew?
				if(1) //The entire crew must X
					switch(rand(1,2)) //What is X?
						if(1) //X is go to Y
							message = "◊À≈Õ€ › »œ¿∆¿ ƒŒÀ∆Õ€ »ƒ“» ¬ [ionarea]"
						if(2) //X is perform Y
							message = "◊À≈Õ€ › »œ¿∆¿ ƒŒÀ∆Õ€ [ionmust]"

				if(2) //A specific crewmember must X
					switch(rand(1,2)) //What is X?
						if(1) //X is go to Y
							message = "[ioncrew1] ƒŒÀ∆≈Õ »ƒ“» ¬ [ionarea]"
						if(2) //X is perform Y
							message = "[ioncrew1] ƒŒÀ∆≈Õ [ionmust]"
			...
			switch(rand(1,2)) //Only or non?
				if(1) //Only human
					switch(rand(1,7)) //Who is it?
						if(1) //A specific job
							message = "“ŒÀ‹ Œ [ioncrew1] ﬂ¬Àﬂ≈“—ﬂ ◊≈ÀŒ¬≈ ŒÃ"
						if(2) //Two specific jobs
							message = "“ŒÀ‹ Œ [ioncrew1] » [ioncrew2] ﬂ¬Àﬂﬁ“—ﬂ Àﬁƒ‹Ã»"
						if(3) //Threats
							message = "“ŒÀ‹ Œ [ionadjectiveshalf][ionthreats] ﬂ¬Àﬂﬁ“—ﬂ Àﬁƒ‹Ã»"
						if(4) // Objects
							message = "“ŒÀ‹ Œ [ionadjectiveshalf][ionobjects] ﬂ¬Àﬂﬁ“—ﬂ Àﬁƒ‹Ã»"
						if(5) // Species
							message = "“ŒÀ‹ Œ [ionspecies] ﬂ¬Àﬂﬁ“—ﬂ Àﬁƒ‹Ã»"
						if(6) //Adjective crewmembers
							message = "“ŒÀ‹ Œ [ionadjectives] Àﬁƒ» ﬂ¬Àﬂﬁ“—ﬂ ◊≈ÀŒ¬≈◊≈— »Ã» —”Ÿ≈—“¬¿Ã»"

						if(7) //Only people who X
							switch(rand(1,3)) //What is X?
								if(1) //X is perform an action
									message = "“ŒÀ‹ Œ “≈  “Œ [ionmust] ﬂ¬Àﬂﬁ“—ﬂ Àﬁƒ‹Ã»"
								if(2) //X is own certain objects
									message = "“ŒÀ‹ Œ “≈ ”  Œ√Œ ≈—“‹ [ionadjectiveshalf][ionobjects] ﬂ¬Àﬂﬁ“—ﬂ Àﬁƒ‹Ã»"
								if(3) //X is eat certain food
									message = "“ŒÀ‹ Œ “≈  “Œ ≈—“‹ [ionadjectiveshalf][ionfood] ﬂ¬Àﬂﬁ“—ﬂ Àﬁƒ‹Ã»"

				if(2) //Non human
					switch(rand(1,7)) //Who is it?
						if(1) //A specific job
							message = "[ioncrew1] Õ≈ ﬂ¬Àﬂ≈“—ﬂ ◊≈ÀŒ¬≈ ŒÃ"
						if(2) //Two specific jobs
							message = "[ioncrew1] » [ioncrew2] Õ≈ ﬂ¬Àﬂﬁ“—ﬂ ◊≈ÀŒ¬≈ ŒÃ"
						if(3) //Threats
							message = "[ionadjectiveshalf][ionthreats] Õ≈ ﬂ¬Àﬂ≈“—ﬂ ◊≈ÀŒ¬≈ ŒÃ"
						if(4) // Objects
							message = "[ionadjectiveshalf][ionobjects] Õ≈ ﬂ¬Àﬂ≈“—ﬂ ◊≈ÀŒ¬≈ ŒÃ"
						if(5) // Species
							message = "[ionspecies] Õ≈ ﬂ¬Àﬂ≈“—ﬂ ◊≈ÀŒ¬≈ ŒÃ"
						if(6) //Adjective crewmembers
							message = "[ionadjectives] Àﬁƒ» Õ≈ ﬂ¬Àﬂ≈“—ﬂ ◊≈ÀŒ¬≈◊≈— »Ã —”Ÿ≈—“¬ŒÃ"
						if(7) //Only people who X
							switch(rand(1,3)) //What is X?
								if(1) //X is perform an action
									message = "“≈  “Œ [ionmust] Õ≈ ﬂ¬Àﬂﬁ“—ﬂ ◊≈ÀŒ¬≈ ŒÃ"
								if(2) //X is own certain objects
									message = "“≈ ”  Œ√Œ ≈—“‹ [ionadjectiveshalf][ionobjects] Õ≈ ﬂ¬Àﬂﬁ“—ﬂ ◊≈ÀŒ¬≈ ŒÃ"
								if(3) //X is eat certain food
									message = "“≈  “Œ ≈—“ [ionadjectiveshalf][ionfood] Õ≈ ﬂ¬Àﬂﬁ“—ﬂ ◊≈ÀŒ¬≈ ŒÃ"
