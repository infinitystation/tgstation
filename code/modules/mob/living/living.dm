...
/mob/living/proc/Exhaust()
	src << "<span class='notice'>Вы слишком измождены, чтобы продолжать...</span>"
	Weaken(5)
...
/mob/living/stripPanelUnequip(obj/item/what, mob/who, where)
	if(what.flags & NODROP)
		src << "<span class='notice'>Вы не можете извлечь [what.name], похоже застрЯло!</span>"
		return
...
/mob/living/stripPanelEquip(obj/item/what, mob/who, where)
	...
	if(what && (what.flags & NODROP))
		src << "<span class='notice'>Вы не можете одеть [what.name] на [who], это застрЯло в вашей руке!</span>"
		return
