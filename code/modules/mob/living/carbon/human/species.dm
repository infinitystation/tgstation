...
/datum/species/proc/handle_hair(mob/living/carbon/human/H, forced_colour)
	...
	if(!hair_hidden || dynamic_hair_suffix)
		...
		S = GLOB.hair_styles_tajaran[H.dna.features["tajaran_hair"]]
		if(S)
	...
	for(var/layer in relevent_layers)
		...
		for(var/bodypart in bodyparts_to_add)
			...
			switch(bodypart)
				...
				if("ears_tajaran")
					S = GLOB.ears_list_tajaran[H.dna.features["ears_tajaran"]]
				if("body_markings")
	...
	switch(slot)
		...
		if(slot_belt)
			...
			if(!H.w_uniform && !nojumpsuit && (!O || O.status != BODYPART_ROBOTIC))
				if(!disable_warning)
					to_chat(H, "<span class='warning'>Вам нужен костюм прежде чем вы сможете присоединить этот [I.name].</span>")
				return 0
		...
		if(slot_s_store)
			...
			if(!H.wear_suit.allowed)
				if(!disable_warning)
					to_chat(H, "У вас каким-то образом костюм без определенных возможностей положить в него предметы, прекратите это.")
				return 0
			if(I.w_class > WEIGHT_CLASS_BULKY)
				if(!disable_warning)
					to_chat(H, "[I.name] слишком большого размера, чтобы это получилось присоединить.")  //should be src?
				return 0
...
/datum/species/proc/handle_chemicals_in_body(mob/living/carbon/human/H)
	...
	if(H.disabilities & FAT)
		if(H.overeatduration < 100)
			to_chat(H, "<span class='notice'>Вы оп&#255;ть чувствуете себ&#255; в норме!</span>")
			H.disabilities &= ~FAT
			H.update_inv_w_uniform()
			H.update_inv_wear_suit()
	else
		if(H.overeatduration > 500)
			to_chat(H, "<span class='danger'>Вы внезапно почувствовали, что вы переели и стали толстеть!</span>")
			H.disabilities |= FAT
			H.update_inv_w_uniform()
			H.update_inv_wear_suit()
	...
	if(H.need_to_shit >= SHIT_LEVEL_MAX)
		H.emote("shit")
		to_chat(H, "<span class='notice'>Вы справили нужду</span>")
	...
	if(H.nutrition > NUTRITION_LEVEL_FAT)
		...
		if(H.metabolism_efficiency != 1.25 && (H.dna && H.dna.species && !(NOHUNGER in H.dna.species.species_traits)))
			to_chat(H, "<span class='notice'>Вы чувствуете себ&#255; энергичным.</span>")
			H.metabolism_efficiency = 1.25
	else if(H.nutrition < NUTRITION_LEVEL_STARVING + 50)
		if(H.metabolism_efficiency != 0.8)
			to_chat(H, "<span class='notice'>Вы чувствуете себ&#255; в&#255;лым.</span>")
		H.metabolism_efficiency = 0.8
	else
		if(H.metabolism_efficiency == 1.25)
			to_chat(H, "<span class='notice'>Вы больше не чувствуете себ&#255; энергичным.</span>")
		H.metabolism_efficiency = 1
		...
		if(H.radiation)
			if (H.radiation > 100)
				if(!H.IsKnockdown())
					H.emote("collapse")
				H.Knockdown(200)
				to_chat(H, "<span class='danger'>Вы чувствуете слабость.</span>")
			switch(H.radiation)
				if(50 to 75)
					if(prob(5))
						if(!H.IsKnockdown())
