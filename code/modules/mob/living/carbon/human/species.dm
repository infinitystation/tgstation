	...
	switch(slot)
		...
		if(slot_belt)
			...
			if(!H.w_uniform && !nojumpsuit)
				if(!disable_warning)
					H << "<span class='danger'>Вам нужен костюм прежде чем вы сможете присоединить этот [I.name].</span>"
				return 0
		...
		if(slot_s_store)
			...
			if(!H.wear_suit.allowed)
				if(!disable_warning)
					H << "У вас каким-то образом костюм без определенных возможностей положить в него предметы, прекратите это."
				return 0
			if(I.w_class > 4)
				if(!disable_warning)
...
/datum/species/proc/handle_chemicals_in_body(var/mob/living/carbon/human/H)
	...
	if(FAT in H.mutations)
		if(H.overeatduration < 100)
			H << "<span class='notice'>Вы опЯть чувствуете себя в норме!</span>"
			H.mutations -= FAT
			H.update_inv_w_uniform(0)
			H.update_inv_wear_suit()
	else
		if(H.overeatduration > 500)
			H << "<span class='danger'>Вы внезапно почувствовали, что вы переели и стали толстеть!</span>"
			H.mutations |= FAT
			H.update_inv_w_uniform(0)
			H.update_inv_wear_suit()
	...
	if(H.nutrition > NUTRITION_LEVEL_FAT)
		...
		if(H.metabolism_efficiency != 1.25)
			H << "<span class='notice'>Вы чувствуете себЯ энергичным.</span>"
			H.metabolism_efficiency = 1.25
	else if(H.nutrition < NUTRITION_LEVEL_STARVING + 50)
		if(H.metabolism_efficiency != 0.8)
			H << "<span class='notice'>Вы чувствуете себЯ вЯлым.</span>"
		H.metabolism_efficiency = 0.8
	else
		if(H.metabolism_efficiency == 1.25)
			H << "<span class='notice'>Вы больше не чувствуете себЯ энергичным.</span>"
		H.metabolism_efficiency = 1
...
/datum/species/proc/handle_mutations_and_radiation(var/mob/living/carbon/human/H)
	...
	if ((HULK in H.mutations) && H.health <= 25)
		...
		H << "<span class='danger'>Внезапно вы почувствовали себЯ ослабленным.</span>"
		H.Weaken(3)
		H.emote("collapse")
				...
				if(50 to 75)
					...
					if(prob(15))
						if(!( H.hair_style == "Shaved") || !(H.hair_style == "Bald") || HAIR in specflags)
							H << "<span class='danger'>Ваши волосы стали выпадать клочьЯми...<span>"
							spawn(50)
...
/datum/species/proc/spec_attacked_by(var/obj/item/I, var/mob/living/user, var/def_zone, var/obj/item/organ/limb/affecting, var/hit_area, var/intent, var/obj/item/organ/limb/target_limb, target_area, var/mob/living/carbon/human/H)
	...
	var/armor = H.run_armor_check(affecting, "melee", "<span class='warning'>Ваша бронЯ защитила вашу [hit_area].</span>", "<span class='warning'>Ваша бронЯ смЯгчила попадание по вашей [hit_area].</span>")
...
/datum/species/proc/handle_temperature(datum/gas_mixture/breath, var/mob/living/carbon/human/H) // called by human/life, handles temperatures
	if( (abs(310.15 - breath.temperature) > 50) && !(COLD_RESISTANCE in H.mutations) && !(COLDRES in specflags)) // Hot air hurts :(
		if(breath.temperature < 260.15)
			if(prob(20))
				H << "<span class='danger'>Вы чувствуете как ваше лицо замерзает, а лёгкие начинают покрыватьсЯ льдом!</span>"
		else if(breath.temperature > 360.15 && !(HEATRES in specflags))
			if(prob(20))
				H << "<span class='danger'>Вы чувствуете, как ваше лицо обжигаетсЯ и в лёгкие заходит жгучаЯ жара!</span>"
