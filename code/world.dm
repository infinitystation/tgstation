...
/world/proc/SetupLogs()
	...
	GLOB.reports_log = file("[GLOB.log_directory]/reports.html")
	GLOB.job_subsystem_debug = file("[GLOB.log_directory]/j-debug.html")
...
/world/proc/load_motd()
	...
	GLOB.join_motd = sanitize_a0(GLOB.join_motd)
	...
	if(SSticker)
		if(SSticker.current_state >= 3)
			if(GLOB.event_on_air)
				features += "<b>EVENT</b>"
			else if(GLOB.master_mode)
				features += "[GLOB.master_mode]"
		else
			if(GLOB.event_on_air)
				features += "<b>STARTING EVENT</b>"
			else
				features += "<b>STARTING [GLOB.master_mode]</b>"
	else
